...
#include <magentart/mlx_gpu_guard.h>
...
bool RealtimeRunner::read_audio_stereo(float* destL, float* destR,
    ...
    if (pending_ring_drain_.exchange(false, std::memory_order_relaxed)) {
        ring_L_.drain();
        ring_R_.drain();
    }
...
void RealtimeRunner::inference_loop() {
    while (running_.load(std::memory_order_relaxed)) {
        ...
        if (do_reset) {
            engine_.reset_state();
            // Drain stale audio on the audio thread instead of resetting ring
            // cursors here — ring reset is undefined while the consumer reads.
            pending_ring_drain_.store(true, std::memory_order_relaxed);
