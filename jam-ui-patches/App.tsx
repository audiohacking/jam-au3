...
function App() {
  ...
  const applyPresetAtIndex = useCallback((list: string[], index: number) => {
    ...
    if (preset) {
      ...
      sendPrompt(preset, true, undefined, true);
  ...
  const sendPrompt = (text: string, immediate = false, soloOverride?: boolean, force = false) => {
    if (isAudioPrompt && !force) return;
  ...
  const isAudioReferenceFile = (file: File) => {
    const ext = file.name.split('.').pop()?.toLowerCase() ?? '';
    return file.type.startsWith('audio/')
      || ['wav', 'wave', 'mp3', 'mpeg', 'm4a', 'aiff', 'aif', 'flac', 'ogg', 'aac'].includes(ext);
  };

  const uploadAudioReference = async (file: File) => {
    if (!isAudioReferenceFile(file)) return;
    ...
    setActiveColor(getPromptColor(file.name));
    setIsPromptEdited(false);
    setIsAudioPrompt(true);
    lastSentText.current = file.name;

    post({
      type: 'loadAudioPromptData',
      index: 0,
      filename: file.name,
      data: base64,
    });
  ...
  const handlePromptDrop = (e: React.DragEvent) => {
    ...
    if (!file || !isAudioReferenceFile(file)) return;
    e.preventDefault();
    e.stopPropagation();
    void uploadAudioReference(file);
  };
  const openSettings = () => {
  ...
  useEffect(() => {
    window.updateState = (state: any) => {
      ...
      if (state.prompt !== undefined && !promptInitialized.current) {
        ...
        // Frontend takes ownership — sync prompt to engine (text only)
        if (!state.isAudioPrompt) {
          sendPrompt(promptToUse, true, solo);
        } else {
          lastSentText.current = promptToUse;
        }
          ...
          <div style={{
            ...
            >
              ...
              <TextField
                ...
                placeholder="Type a prompt or drop a WAV/MP3 reference."
              ...
              <div
                ...
                {isAudioPrompt ? (
                  ...
                  <>
                    <Tooltip title="Upload audio reference (WAV, MP3)" placement="top">
                      <IconButton
                        variant="jam"
                        onClick={loadAudioPrompt}
                        sx={{ width: 36, height: 36 }}
                      >
                        <AudioFile sx={{ fontSize: 18 }} />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Choose audio file from disk" placement="top">
                      <IconButton
                        variant="jam"
                        onClick={loadAudioPrompt}
                        sx={{ width: 36, height: 36 }}
